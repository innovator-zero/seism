...
if ~exist(db_root_dir(database),'dir')
    error(['Database root dir ''' db_root_dir(database) ''' not found']);
end
...
elseif strcmp(database,'PASCALContext')
    gt = loadvar(fullfile(db_root_dir(database), [image_id '.mat']),'LabelMap');
    ground_truth = {relabel(uint32(gt))};
elseif strcmp(database, 'NYUD')
    gt = loadvar(fullfile(db_root_dir(database), [image_id '.mat']), 'groundTruth');
    ground_truth = cell(1,1);
    ground_truth{1} = gt{1}.Boundaries;
else
